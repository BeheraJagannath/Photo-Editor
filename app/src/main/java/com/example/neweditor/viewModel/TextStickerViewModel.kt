...
import com.example.neweditor.customView.strickerView.Sticker
import com.example.neweditor.customView.strickerView.TextSticker
import com.example.neweditor.data.*
import com.example.neweditor.databinding.FragmentStickertextBinding
import com.example.neweditor.enmus.ActiveNavArgsData
import com.example.neweditor.fragment.ColorViewPagerFragment
import com.example.neweditor.fragment.FontViewPagerFragment
...
class TextStickerViewModel(
    ...
    navArgs: StickerTextFragmentArgs,
     ...
     */
    private val _currentSticker = MutableLiveData<Sticker>()
    val currentSticker: LiveData<Sticker>?
        get() {
    ...
    val colorPositionInRecyclerView = Transformations.map(_currentSticker) { currentSticker ->
        val convertToTextSticker = currentSticker as TextSticker
...
//        _imgSrc.value = imgSrcBitmap
    ...
    private fun setUriOrBitmap(navArgs: StickerTextFragmentArgs) {
    ...
    fun updateCurrentSticker(sticker:Sticker) {
    ...
    private fun checkIsStickerInsideList(textSticker: TextSticker): Boolean? {
    ...
    fun addOneStickerInList(textSticker: TextSticker) {
    ...
    fun deleteOneStickerFromList(textSticker: TextSticker) {
    ...
    fun updateOneStickerInList(textSticker: TextSticker) {
     ...
     * @return [TextSticker]
     */
    fun createNewSticker(textStr: String): TextSticker {
        return TextSticker(context).apply {
    ...
    fun changeStickerColor(color: Int, positionInRecyclerView: Int) {
        ...
        val textSticker = _currentSticker.value as  TextSticker
        val element = _stickersInfoList.value?.find { it.id == textSticker.id }
        ...
        val sticker = textSticker as  Sticker
        updateStickerView(sticker)

    }

    fun updateStickerView(sticker:Sticker) {
